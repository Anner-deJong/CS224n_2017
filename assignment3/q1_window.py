...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        T = len(sentence)
        full_sent = [start]*window_size + sentence + [end]*window_size
        for t in range(T):
            window = sum(full_sent[t:t+1+2*window_size], [])
            windowed_data.append((window, labels[t]))
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        concatenates those vectors:
            ...
            - Concatenates the embeddings by reshaping the embeddings tensor to shape
              (None, n_window_features * embed_size).
    ...
    def add_prediction_op(self):
        ...
        W_size = (self.config.n_window_features * self.config.embed_size, self.config.hidden_size)
        W      = tf.get_variable('W',  initializer=tf.contrib.layers.xavier_initializer(), shape=W_size)
        b1     = tf.get_variable('b1', initializer=tf.contrib.layers.xavier_initializer(), shape=self.config.hidden_size)
