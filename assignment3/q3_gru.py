...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self): 
        ...
        if self.config.cell == "rnn":
            ...
            cell = LSTMCell(1, 1) # adjust this for tensorflow V 1.0.1 (possibly earlier on as well)
        else:
        ...
        outputs, state = tf.nn.dynamic_rnn(cell=cell, inputs=x, dtype=tf.float32) #, sequence_length=self.config.max_length)
    ...
    def add_training_op(self, loss):
        ...
        unclip_grads_vars = optimizer.compute_gradients(loss)

        if self.config.clip_gradients:
            grads, grad_norm = tf.clip_by_global_norm(t_list=[t[0] for t in unclip_grads_vars], clip_norm=self.config.max_grad_norm)
            grads_vars       = [(grads[i], t[1]) for i, t in enumerate(unclip_grads_vars)]
            #for i in range(len(grads_vars)):
            #    print(type(grads_vars[i]))
            #    grads_vars[i][0] = grads[i]

        else:
            grads_vars     = unclip_grads_vars

        self.grad_norm = tf.global_norm([t[0] for t in grads_vars])
...
def make_dynamics_plot(args, x, h, ht_rnn, ht_gru, params):
    ...
    Ur, Wr, br, Uz, Wz, Wh, Wx, bz, Uo, Wo, bo, b = params

    plt.clf()
    plt.title("""Cell dynamics when x={}:
    Ur={:.2f}, Wr={:.2f}, br={:.2f}, Uz={:.2f}, Wz={:.2f}, bz={:.2f}, Uo={:.2f}, Wo={:.2f}, bo={:.2f},
    Urnn={:.2f}, Wrnn={:.2f}, brnn={:.2f}""".format(x, Ur[0,0], Wr[0,0], br[0], Uz[0,0], Wz[0,0], bz[0], Uo[0,0], Wo[0,0], bo[0], Wx[0,0], Wh[0,0], b[0]))
    ...
    with tf.Graph().as_default():
        ...
        with tf.variable_scope("dynamics"):
            ...
            with tf.variable_scope("cell"):
                Ur, Wr, Uz, Wz, Uo, Wo, Wh, Wx = [mat(3*x) for x in np.random.randn(8)]
                br, bz, bo, b = [vec(x) for x in np.random.randn(4)]
                params = [Ur, Wr, br, Uz, Wz, Wh, Wx, bz, Uo, Wo, bo, b]
                ...
                tf.get_variable("W_x", initializer=Wx)
                tf.get_variable("W_h", initializer=Wh)
                tf.get_variable("b",   initializer=b)

            tf.get_variable_scope().reuse_variables()
            y_gru, h_gru = GRUCell(1,1)(x_placeholder, h_placeholder, scope="cell")
            y_rnn, h_rnn = RNNCell(1,1)(x_placeholder, h_placeholder, scope="cell") # changed from GRUCell to RNNCell
