...
class Parser(object):
    ...
    def extract_features(self, stack, buf, arcs, ex):
        ...
        for i in range(2): # Anner: changed xrange into range
            if i < len(stack):
                k = stack[-i-1]
                lc = get_lc(k)
                rc = get_rc(k)
    ...
    def create_instances(self, examples):
        ...
        for id, ex in enumerate(logged_loop(examples)):
            ...
            # arcs = {(h, t, label)}
            stack = [0]
            buf = [i + 1 for i in range(n_words)] # Anner: changed xrange into range
            arcs = []
            instances = []
            for i in range(n_words * 2): # Anner: changed xrange into range
