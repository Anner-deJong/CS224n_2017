...
class Config(object):
    ...
    dropout     = 0.5
    embed_size  = 50
    hidden_size = 200   # original value: 200
    batch_size  = 2048
    n_epochs    = 10    # original value: 10
    lr          = 0.001
...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        #embeddings=tf.contrib.layers.flatten(tf.nn.embedding_lookup(self.pretrained_embeddings, self.input_placeholder))
    ...
    def run_epoch(self, sess, parser, train_examples, dev_set):
        ...
        print("Evaluating on dev set") # deleted another comma??
...
def main(debug=False):
    ...
    with tf.Graph().as_default():
        print("Building model...") # deleted a comma??
        start = time.time()
