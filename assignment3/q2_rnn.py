...
from q4_lstm_cell import LSTMCell
...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        sen = sentence[:max_length]               + [zero_vector] * (max_length - len(sentence))
        lab = labels[:max_length]                 + [zero_label]  * (max_length - len(sentence))
        msk = [True] * len(sentence[:max_length]) + [False]       * (max_length - len(sentence))

        ret.append((sen, lab, msk))
...
class RNNModel(NERModel):
    ...
    def create_feed_dict(self, inputs_batch, mask_batch, labels_batch=None, dropout=1):
        ...
        if (labels_batch != None): feed_dict[self.labels_placeholder] = labels_batch
    ...
    def add_prediction_op(self):
        ...
        U_size     = (self.config.hidden_size, self.config.n_classes)
        U          = tf.get_variable('U',  initializer=tf.contrib.layers.xavier_initializer(), shape=U_size)
        b2         = tf.get_variable('b2', initializer=tf.contrib.layers.xavier_initializer(), shape=self.config.n_classes)
        batch_size = tf.shape(x)[0] # = self.config.batch_size
        state      = tf.zeros((batch_size, self.config.hidden_size))
        output     = tf.zeros((batch_size, self.config.hidden_size))

        ### END YOUR CODE

        with tf.variable_scope("RNN"):

            output, state = cell(x[:, 0, :], state, output)
            out_drop      = tf.nn.dropout(output, dropout_rate)
            pred          = tf.add(tf.matmul(out_drop, U), b2)

            preds.append(pred)

            for time_step in range(1, self.max_length): # changed to exclude first loop
                ### YOUR CODE HERE (~6-10 lines)

                #if time_step > 0: # first loop added above!

                tf.get_variable_scope().reuse_variables()
                output, state = cell(x[:, time_step, :], state, output)
                out_drop      = tf.nn.dropout(output, dropout_rate)
                pred          = tf.add(tf.matmul(out_drop, U), b2)
    ...
    def add_loss_op(self, preds):
        ...
        # masked only 1x, after computing the CE
        # allowed since CE remains its shape, and each index(=token) is not influencing other indices for softmax_CE
        #   (since the softmax iterates for each label index only over the predictions for that specific label index)
        # could be faster to mask before? results in masking 2x (both labels AND predictions)
        #   (or instead of masking, yet still requiring some sort of mask:
        #    set predictions to one hot according to labels, most likely more expensive)
...
def do_test2(args):
    ...
    with tf.Graph().as_default():
        ...
        #]
        #ret_ = pad_sequences(data, 4)
        #ret__ = model.preprocess_sequence_data(data)
        #print(ret__)
        #exit()
...
def do_train(args):
    ...
    print("time elapsed: {:.2f}".format(time.clock() - tic))
